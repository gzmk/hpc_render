...
writetable(T,'/scratch/gk925/hpc_render/render_scene5/bumpy_fitrender_Conditions.txt','Delimiter','\t')

% Set preferences
setpref('RenderToolbox3', 'workingFolder', '/scratch/gk925/hpc_render/render_scene5');

for bump = 0.4:0.4:4.4
    bump_level = strrep(num2str(bump), '.', '_');
    % use this scene and condition file. 
    parentSceneFile = ['bumpy',bump_level,'_5by5_correctCameraDist5.dae']
    conditionsFile = 'bumpy_fitrender_Conditions.txt';
    mappingsFile = ['bumpy',bump_level,'_5by5_correctCameraDist5DefaultMappings.txt'];
%     mappingsFile = 'bumpy5_hemilight_ortho2DefaultMappings.txt'
    ...
    for renderer = {'Mitsuba'}
        ...
        montageName = sprintf('Bump%s_Gloss%s_Scene5', bump_level, num2str(gloss_level));
