...
diffuse = mean(correctedForNorm(:,2));

ro_s = ['300:',num2str(var(1)),' 800:',num2str(var(1))];
ro_d = ['300:', num2str(diffuse), ' 800:', num2str(diffuse)];
...
writetable(T,'/scratch/gk925/hpc_render/render_spheronFit_LF2/bumpy_fitrender_Conditions.txt','Delimiter','\t')

% Set preferences
setpref('RenderToolbox3', 'workingFolder', '/scratch/gk925/hpc_render/render_spheronFit_LF2');

for bump = 0.4:0.4:4.4
    bump_level = strrep(num2str(bump), '.', '_');
    % use this scene and condition file. 
    parentSceneFile = ['bumpy',bump_level,'_5by5_correctCameraDist2.dae']
    conditionsFile = 'bumpy_fitrender_Conditions.txt';
    mappingsFile = ['bumpy',bump_level,'_5by5_correctCameraDist2DefaultMappings.txt'];
%     mappingsFile = 'bumpy5_hemilight_ortho2DefaultMappings.txt'
    ...
    hints.recipeName = ['Bump-',bump_level,'-','Gloss-',num2str(gloss_level),'-Scene2-' datetime];
    ...
    for renderer = {'Mitsuba'}
        ...
        montageName = sprintf('Bump%s_Gloss%s_Scene2', bump_level, num2str(gloss_level));
